...
if(isset($_POST['update'])) {
   ...
   echo  $newicon='images/project/'.basename($_FILES["icon"]["name"]);
    if($_SESSION['pre_icon']==$newicon) {
        echo 'file are same ';
        header('location:../case_study_detail.php?action=done');
 ...
 if( $_FILES['icon']['name'] == '') {
     $dbpath=$_SESSION['pre_icon'];
            echo 'file is empty';
                   $sql="UPDATE case_study_detail SET b_id='$_POST[b_id]',title='$_POST[title]',description='$_POST[description]',image='$dbpath' WHERE id='$_POST[id]'";
