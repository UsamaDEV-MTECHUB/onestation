...
$sql="SELECT * FROM case_study_detail WHERE id='$_GET[id]'";
...
echo '<form class="form fv-plugins-bootstrap fv-plugins-framework" novalidate="novalidate" action="case_study_detail/update_about_gallery.php" method="POST" enctype="multipart/form-data">
<div class="modal-body" id="servicearea">
  ...
  <?php
                                    ...
                                    $sqll="SELECT * FROM vertical";
                                    $runl=mysqli_query($conn,$sqll);
                                    while($rowl=mysqli_fetch_assoc($runl)) {
                                        if ($b_id==$rowl['id']) {
                                        echo '<option value="'.$rowl['id'].'" selected>'.$rowl['heading'].'</option>';    
                                        }
                                        else {
                                            echo '<option value="'.$rowl['id'].'">'.$rowl['heading'].'</option>';
                                        }
